...
# Load data
reorder_df = pd.read_csv('reorder_report.csv')
forecast_df = pd.read_csv('forecasts.csv', parse_dates=['ds'])
demand_df = pd.read_csv('weekly_demand.csv', parse_dates=['Week'])
...
if page == "Stock Overview":
    ...
    fig = go.Figure()
    fig.add_bar(
        x=reorder_df['Product'],
        y=reorder_df['Current Stock (simulated)'],
        name='Current Stock',
        marker_color='#378ADD'
    )
    fig.add_bar(
        x=reorder_df['Product'],
        y=reorder_df['Reorder Point'],
        name='Reorder Point',
        marker_color='#E24B4A',
        opacity=0.6
    )
    ...
    fig2 = go.Figure(go.Bar(
        x=reorder_df['Product'],
        y=reorder_df['Avg Weekly Demand'],
        marker_color='#1D9E75'
    ))
    fig2.update_layout(
        xaxis_tickangle=-30,
...
# ── PAGE 2: DEMAND FORECAST ─────────────────────────────────────────
    ...
    actual_only = product_fc[product_fc['actual'].notna()]
    forecast_only = product_fc[product_fc['actual'].isna()]

    # Get accuracy from reorder_df
    code = reorder_df[reorder_df['Product'] == selected]['StockCode'].values
    ...
    fig.add_trace(go.Scatter(
        x=actual_only['ds'], y=actual_only['actual'],
        name='Actual demand', line=dict(color='#378ADD', width=2)
    ))
    fig.add_trace(go.Scatter(
        x=product_fc['ds'], y=product_fc['yhat'],
        name='Forecast', line=dict(color='#1D9E75', width=2, dash='dash')
    ))
    fig.add_vline(
        x=actual_only['ds'].max(),
        line_dash='dot', line_color='#E24B4A',
...
# ── PAGE 3: REORDER ALERTS ──────────────────────────────────────────
    ...
    display_cols = [
        'Product', 'Avg Weekly Demand', 'Safety Stock',
        'Reorder Point', 'Current Stock (simulated)',
        'Forecast Next 4 Weeks', 'Recommended Order Qty', 'Status'
    ]

    styled = reorder_df[display_cols].style.applymap(
